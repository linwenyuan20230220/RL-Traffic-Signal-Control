...
# === Network 結構 (無須變動，buffer 會隨 model.to(device) 自動移動) ===
class CoLightGATNetwork(nn.Module):
    def __init__(self, num_agents, state_dim, action_dim, adj_matrix, hidden_dim=128, heads=5):
        super(CoLightGATNetwork, self).__init__()
        
        # adj_matrix 註冊為 buffer，當模型 .to(device) 時它會自動跟著走
    ...
    def forward(self, x):
        ...
        q = self.W_q(h).view(batch_size, num_agents, self.heads, self.head_dim)
        k = self.W_k(h).view(batch_size, num_agents, self.heads, self.head_dim)
        v = self.W_v(h).view(batch_size, num_agents, self.heads, self.head_dim)
        ...
        mask = self.adj_mask.unsqueeze(1) 
        ...
        agg_feats = attended_values.view(batch_size, num_agents, self.hidden_dim)

        agg_feats = agg_feats + h

        values = self.value_stream(agg_feats)
        advantages = self.advantage_stream(agg_feats)
        
        q_values = values + (advantages - advantages.mean(dim=2, keepdim=True))
        return q_values

# === Agent (GPU 修改重點區域) ===
class COGATDQN(Agent):
    def __init__(self, obs_act):
        ...
        if torch.cuda.is_available():
            ...
            print(f"[系統] 使用 GPU 加速: {torch.cuda.get_device_name(0)}")
        elif torch.backends.mps.is_available():
            self.device = torch.device("mps") # Mac M1/M2/M3
            print("[系統] 使用 Apple MPS 加速")
        else:
        ...
        self.model = CoLightGATNetwork(len(self.agent_ids), self.state_dim, self.action_dim, self.adj, hidden_dim=128, heads=4)
    ...
    def _pad_state(self, state_dict):
        ...
        # 注意：這裡先回傳 CPU Tensor，到 act 裡面再搬移
        return torch.FloatTensor(np.array(padded_list)).unsqueeze(0)

    def act(self, observation):
        self.last_state = observation
        # [GPU Step 3] 將 State 搬移到 Device
        state_tensor = self._pad_state(observation).to(self.device)
        ...
        for i, agent_id in enumerate(self.agent_ids):
            ...
            if is_random:
                ...
                # 這裡 q_values 在 GPU 上，取值後 .item() 會自動轉回 Python float
    ...
    def observe(self, next_observation, reward, done, info):
        ...
        for agent_id in self.agent_ids:
            s = np.array(self.last_state[agent_id], dtype=np.float32)
            pad_len = self.state_dim - len(s)
            if pad_len > 0: s = np.pad(s, (0, pad_len), 'constant')
            s_list.append(s)

            ns = np.array(next_observation[agent_id], dtype=np.float32)
            pad_len = self.state_dim - len(ns)
            if pad_len > 0: ns = np.pad(ns, (0, pad_len), 'constant')
            next_s_list.append(ns)
            
            r_list.append(scaled_reward[agent_id])
            a_list.append(self.last_actions[agent_id])
            
        self.memory.append((s_list, a_list, r_list, next_s_list, done))
    ...
    def _replay(self):
        ...
        with torch.no_grad():
            ...
            # torch.full_like 會自動產生在同一個 device 的 tensor
