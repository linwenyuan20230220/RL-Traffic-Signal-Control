...
引入強基準： 引入 [CIKM 2019] 的 CoLight 模型，其利用圖注意力網路 (GAT) 實現路口間的空間協作。
...
本專案基於 RESCO 開源框架進行二次開發。

原始框架貢獻： [J. Ault et al., NeurIPS 2021]

本專案新增貢獻： 1. 實作「視野受限」與「資訊延遲」的環境干擾模擬器。
2. 自主設計並實作 STGATDQN 模型架構（結合 Temporal Attention 與 GAT）。
